...
class CheckPointMixin:
    ...
    def load_only_checkpoint(self, model, load_arg='load', strict=True):
        args = get_args()
        ...
        if args.lora:
            ...
            if len(model) == 1:
                poped_ckpt = {k: v for k, v in state_dict['model'].items() if k in model[0].state_dict() and v.shape != model[0].state_dict()[k].shape}
                print_rank_0("WARNING: pop keys for mismatch dimension, " + str(poped_ckpt.keys()))
                for k in poped_ckpt.keys(): state_dict['model'].pop(k)
                missing_in_ckpt = [
                    k for k in model[0].state_dict().keys() if k not in state_dict['model']
                ]
                if len(missing_in_ckpt) > 0:
                    print_rank_0("WARNING: parameters in model but NOT in checkpoint: " + str(missing_in_ckpt))
                model[0].load_state_dict(state_dict['model'], strict=False)
            else:
                for i in range(len(model)):
                    mpu.set_virtual_pipeline_model_parallel_rank(i)
                    poped_ckpt = {k: v for k, v in state_dict['model%d' % i].items() if k in model[i].state_dict() and v.shape != model[i].state_dict()[k].shape}
                    print_rank_0("WARNING: pop keys for mismatch dimension, "+ str(poped_ckpt.keys()))
                    for k in poped_ckpt.keys(): state_dict['model%d' % i].pop(k)
                    missing_in_ckpt = [
                        k for k in model[i].state_dict().keys() if k not in state_dict[f'model{i}']
                    ]
                    if len(missing_in_ckpt) > 0:
                        print_rank_0(f"WARNING: [stage {i}] parameters in model but NOT in checkpoint: {missing_in_ckpt}")
    ...
    def load_checkpoint(self, model, optimizer, opt_param_scheduler, load_arg='load', strict=True, ema_models=None):
        ...
        # Optimizer.
        if not release and not args.finetune and not args.no_load_optim and load_arg == "load":
            try:
                # Load state dict.
        ...
        # rng states.
        if not release and not args.finetune and not args.no_load_rng and load_arg == "load":
            try:
