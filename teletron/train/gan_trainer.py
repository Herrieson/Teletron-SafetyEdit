...
from teletron.core.pipeline_parallel_gan import get_forward_backward_func
...
class GANTrainer(CheckPointMixin, SchedulerMixin, DataloaderMixin, TeleLoggerMixin):
    def __init__(
        self,
        args,
        ...
        self.model_g, self.model_d, self.optimizers, self.schedulers, self.ema_models = \
                                self.setup_model_and_optimizer(args.model_type)


        self.train_itrt, self.valid_itrt, self.test_itrt = \
                                self.get_iterator(len(self.model_g), dataset_provide_func)
        ...
        self.test_itrt =  None
        self.config = get_model_config(self.model_g[0])
    ...
    def setup_model_and_optimizer(self,  
        ...
        if args.use_zero2:
            pass
            # model_g = self.get_model(model_type, name='dit', wrap_with_ddp=False)
        else:
            model_g = self.get_model(model_type, name='dit')
            model_d = self.get_model(model_type, name='ddit')

        unwrapped_model_g = unwrap_model(model_g)
        unwrapped_model_d = unwrap_model(model_d)
        ...
        if args.use_zero2:
            ...
            optimizer_g = self.get_optimizer(config, model_g, no_wd_decay_cond,
                                        scale_lr_cond, lr_mult)
            scheduler_g = self.get_optimizer_param_scheduler(optimizer_g)

            optimizer_d = self.get_optimizer(config, model_d, no_wd_decay_cond,
                                        scale_lr_cond, lr_mult)
            scheduler_d = self.get_optimizer_param_scheduler(optimizer_d)

            optimizers = {'gen': optimizer_g, 'disc': optimizer_d}
            schedulers = {'gen': scheduler_g, 'disc': scheduler_d}
        # === 关键改法结束 ===

        if args.load is not None:
            if isinstance(optimizers, dict):
                args.iteration, args.num_floating_point_operations_so_far, optimizers['gen'], schedulers['gen'] = \
                    self.load_checkpoint(model_g, optimizers['gen'], schedulers['gen'], strict=False)
                _, _, optimizers['disc'], schedulers['disc'] = \
                    self.load_checkpoint(model_d, optimizers['disc'], schedulers['disc'], load_arg='load_disc', strict=False)
            else:
                pass
        ...
        if args.iteration == 0 and len(unwrapped_model_g) == 1 \
            and hasattr(unwrapped_model_g[0], 'init_state_dict_from_bert'): # 不执行
            print_rank_0("Initializing ICT from pretrained BERT model")
            unwrapped_model_g[0].init_state_dict_from_bert()
            unwrapped_model_d[0].init_state_dict_from_bert()
            if args.fp16 and not isinstance(optimizers, dict):
                optimizers['gen'].reload_model_params()
                optimizers['disc'].reload_model_params()
        ...
        # opt = optimizers['gen']
        # print(f"[gen] OUTER = {type(opt).__name__}")
        # if hasattr(opt, "param_groups"):
        #     for i, g in enumerate(opt.param_groups):
        #         tensors_cnt = 0
        #         elems_cnt = 0
        #         shown = 0
        #         print(f"  - group{i}: lr={g.get('lr')}, weight_decay={g.get('weight_decay')}")
        #         if 'params' in g:
        #             for p in g['params']:
        #                 # 统计当前 rank 持有的本地分片/参数
        #                 n = None
        #                 if isinstance(p, torch.Tensor):
        #                     n = int(p.numel())
        #                 else:
        #                     # 常见分片包装尝试取底层 tensor
        #                     for attr in ("data", "tensor", "fp32_shard", "payload"):
        #                         t = getattr(p, attr, None)
        #                         if isinstance(t, torch.Tensor):
        #                             n = int(t.numel())
        #                             break
        #                     if n is None and hasattr(p, "numel"):
        #                         try:
        #                             n = int(p.numel())
        #                         except Exception:
        #                             n = None
        ...
        #                     print(f"      param[{shown}] n={n}" + (f", shape={tuple(shape)}" if shape is not None else ""))
        #                     #shown += 1
        #         print(f"    => tensors={tensors_cnt}, n_params={elems_cnt}")
        # else:
        #     print("  (no param_groups on outer optimizer)")

        # opt = optimizers['disc']
        # print(f"[disc] OUTER = {type(opt).__name__}")
        # if hasattr(opt, "param_groups"):
        #     for i, g in enumerate(opt.param_groups):
        #         tensors_cnt = 0
        #         elems_cnt = 0
        ...
        # # 尝试剥一层，看看底层优化器（若存在）
        # base = opt
        # for attr in ("optimizer", "_optimizer", "optim", "_optim", "base_optimizer", "inner_optimizer"):
        #     if hasattr(base, attr) and isinstance(getattr(base, attr), torch.optim.Optimizer):
        #         base = getattr(base, attr)
        #         break

        # if base is not opt:
        #     print(f"[gen] INNER = {type(base).__name__}")
        #     if hasattr(base, "param_groups"):
        #         for i, g in enumerate(base.param_groups):
        #             cnt = 0
        #             tot = 0
        ...
        #                     cnt += 1
        #                     tot += int(p.numel())
        #             print(f"  - group{i}: lr={g.get('lr')}, weight_decay={g.get('weight_decay')}, tensors={cnt}, n_params={tot}")
        #     else:
        #         print("  (no param_groups on inner optimizer)")




        return model_g, model_d, optimizers, schedulers, ema_models
        ...
        if not args.skip_train:
            ...
            if args.do_train and args.train_iters > 0:
                iteration, num_floating_point_operations_so_far = self.train(
                    forward_step_func,
                    self.model_g, self.model_d, self.optimizers, self.schedulers,
    ...
    def train(
        ...
        # -------- wrap_forward：把 GANForwardStep 适配为 Megatron 期待的 forward_step_fn --------

        model_g_chunks = model_g # as_chunks(model_g)
        model_d_chunks = model_d # as_chunks(model_d)

        for mm in (model_g_chunks + model_d_chunks):
            mm.train()
        total_loss_dict = {}
        ...
        while iteration < args.train_iters:
            ...
            if get_num_microbatches() != num_microbatches and iteration != 0:
                ...
                self.save_checkpoint_and_time(
                    iteration, model_g_chunks, optimizers['gen'],
                    opt_param_schedulers['gen'],
            ...
            if isinstance(model_g_chunks[0], DDP) and args.overlap_grad_reduce:
                assert cfg_g.no_sync_func is None, \
                    ('When overlap_grad_reduce is True, config.no_sync_func must be None; '
                     'a custom no_sync_func is not supported when overlapping grad-reduce')
                cfg_g.no_sync_func = [mc.no_sync for mc in model_g_chunks]
                if len(model_g_chunks) == 1:
                    cfg_g.no_sync_func = cfg_g.no_sync_func[0]
                if args.delay_grad_reduce:
                    cfg_g.grad_sync_func = [mc.start_grad_sync for mc in model_g_chunks]
                    if len(model_g_chunks) == 1:
            ...
            if args.overlap_param_gather and args.delay_param_gather:
                cfg_g.param_sync_func = [lambda x: optimizers['gen'].finish_param_sync(mi, x)
                                         for mi in range(len(model_g_chunks))]
                if len(model_g_chunks) == 1:
            ...
            loss_dict_g, skipped_g, grad_norm_g, zeros_g = self.train_step(
                fwd_gen, 'gen', train_data_iterator,
                model_g_chunks, model_d_chunks, optimizers['gen'],
                opt_param_schedulers['gen'], cfg_g, ema_models
            )
            #print("loss_dict_g =", loss_dict_g)
            if grad_norm_g is None:
                if args.use_zero2:
                    grad_norm_g = optimizers['gen']._global_grad_norm
                else:
                    grad_norm_g = get_grad_norm(optimizers['gen'])

            # 记录 G 的 LR 与日志
            if args.use_zero2:
                loss_scale_g = optimizers['gen']._get_loss_scale()
            else:
                loss_scale_g = optimizers['gen'].get_loss_scale().item()
            lr_g = lr_g_dec = None
            for pg in optimizers['gen'].param_groups:
            ...
            #self.print_trainable_params()
            #################################################################
            cfg_d = copy.copy(base_config)
            if isinstance(model_d_chunks[0], DDP) and args.overlap_grad_reduce:
                assert cfg_d.no_sync_func is None, \
                    ('When overlap_grad_reduce is True, config.no_sync_func must be None; '
                     'a custom no_sync_func is not supported when overlapping grad-reduce')
                cfg_d.no_sync_func = [mc.no_sync for mc in model_d_chunks]
                if len(model_d_chunks) == 1:
                    cfg_d.no_sync_func = cfg_d.no_sync_func[0]
                if args.delay_grad_reduce:
                    cfg_d.grad_sync_func = [mc.start_grad_sync for mc in model_d_chunks]
                    if len(model_d_chunks) == 1:
                        cfg_d.grad_sync_func = cfg_d.grad_sync_func[0]
            if args.overlap_param_gather and args.delay_param_gather:
                cfg_d.param_sync_func = [lambda x: optimizers['disc'].finish_param_sync(mi, x)
                                         for mi in range(len(model_d_chunks))]
                if len(model_d_chunks) == 1:
                    cfg_d.param_sync_func = cfg_d.param_sync_func[0]

            fwd_disc = forward_step_func.forward_step #wrap_forward(forward_step_func, 'disc')
            loss_dict_d, skipped_d, grad_norm_d, zeros_d = self.train_step(
                fwd_disc, 'disc', train_data_iterator,
                model_g_chunks, model_d_chunks, optimizers['disc'],
                opt_param_schedulers['disc'], cfg_d, ema_models
            )

            if grad_norm_d is None:
                if args.use_zero2:
                    grad_norm_d = optimizers['disc']._global_grad_norm
                else:
                    grad_norm_d = get_grad_norm(optimizers['disc'])

            # D 日志
            if args.use_zero2:
                loss_scale_d = optimizers['disc']._get_loss_scale()
            else:
                loss_scale_d = optimizers['disc'].get_loss_scale().item()
            lr_d = lr_d_dec = None
            for pg in optimizers['disc'].param_groups:
                if pg['is_decoupled_lr']: lr_d_dec = pg['lr']
                else: lr_d = pg['lr']
            report_memory_flag = self.log_training_infos(
                loss_dict_d, total_loss_dict,
                lr_d, lr_d_dec, iteration, loss_scale_d,
                report_memory_flag, skipped_d, grad_norm_d, None, zeros_d
            )
            #################################################################
            # ### for real ###
            # cfg_d = copy.copy(base_config)
            ...
            # fwd_disc = forward_step_func.forward_step #wrap_forward(forward_step_func, 'disc')
            # loss_dict_d, skipped_d, grad_norm_d, zeros_d = self.train_step(
            #     fwd_disc, 'disc_real', train_data_iterator,
            #     model_g_chunks, model_d_chunks, optimizers['disc'],
            #     opt_param_schedulers['disc'], cfg_d, ema_models
            # )
            ...
            # fwd_disc = forward_step_func.forward_step #wrap_forward(forward_step_func, 'disc')
            # loss_dict_d, skipped_d, grad_norm_d, zeros_d = self.train_step(
            #     fwd_disc, 'disc_fake', train_data_iterator,
            #     model_g_chunks, model_d_chunks, optimizers['disc'],
            #     opt_param_schedulers['disc'], cfg_d, ema_models
            # )
            ...
            # )
            #################################################################
            # —— 本轮只真正消费了一批数据（G 推进迭代器，D 复用缓存），故样本数只加一次；
            #    FLOPs 需要包含 G 和 D 两个优化步：乘以 2。
            iteration += 1
            ...
            if args.eval_interval and iteration % args.eval_interval == 0 and \
                ...
                self.evaluate_and_print_results(
                    ...
                    valid_data_iterator, model_g_chunks,
                ...
                if args.use_distributed_optimizer and args.overlap_param_gather:
                    optimizers['gen'].enable_pre_hook()
    ...
    def train_step(
        ...
        if not args.use_zero2:
            for model_chunk in model_g:
                model_chunk.zero_grad_buffer()
            for model_chunk in model_d:
                model_chunk.zero_grad_buffer()
