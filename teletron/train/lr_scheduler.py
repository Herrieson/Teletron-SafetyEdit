...
# ---------- utils for printing optimizable parameter keys ----------
def _unwrap_module(m):
    return getattr(m, "module", m)

def _collect_named_params_from_model_list(model_list):
    """
    返回:
      name2param: "model[i].<param_name>" -> param
      id2name:    id(param) -> "model[i].<param_name>"
    """
    name2param, id2name = {}, {}
    if model_list is None:
        return name2param, id2name
...
def _extract_param_ids_from_groups(param_groups):
    """从 param_groups 提取参数 id，并标记 lr_mult=0 的 id。"""
    ids_all, ids_lr0 = set(), set()
    for g in param_groups:
        ...
        for p in g.get("params", []):
            ...
            if lr_mult == 0.0:
                ids_lr0.add(pid)
    return ids_all, ids_lr0

def print_optimizable_keys(tag, param_groups, id2name):
    """
    打印“会被优化”的参数 key：
      - 在 param_groups 里
      - requires_grad=True
    ...
    """
    if not param_groups:
        print(f"{tag}: <empty param_groups>")
        return
    ids_all, ids_lr0 = _extract_param_ids_from_groups(param_groups)
    keys = []
    for g in param_groups:
        lr_mult = g.get("lr_mult", 1.0)
        if lr_mult == 0.0:
        ...
        for p in g.get("params", []):
            if not getattr(p, "requires_grad", False):
                continue
            pid = id(p)
            if (pid in ids_all) and (pid not in ids_lr0):
                name = id2name.get(pid)
                if name is not None:
                    keys.append(name)
    uniq = sorted(set(keys))
    print(f"{tag} optimizable parameter keys (count={len(uniq)}):")
    for k in uniq:
        print(f"  {k}")
...
class SchedulerMixin:
    ...
    def get_optimizer_for_zero2(self, config: OptimizerConfig, 
        ...
        param_names = {name: param for name, param in model[0].named_parameters() if ("compressor" not in name) and ("quantizer" not in name)}
    ...
    def get_optimizer(self, config: OptimizerConfig, 
        ...
        def freeze_keywords_lr0(scale_lr_cond=None, keywords=("compressor", "quantizer")):
            def wrapped(name, param):
                if any(k in name for k in keywords):
                    return 0.0  # 让组建 param_groups 时这类参数 lr 为 0
                return 1.0 if scale_lr_cond is None else scale_lr_cond(name, param)
            return wrapped
        scale_lr_cond = freeze_keywords_lr0(scale_lr_cond)
        # Collect param groups.
        param_groups = _get_param_groups(
            model,
            no_weight_decay_cond,
        ...
        # === 新增：打印普通 Megatron 路径下的可优化参数 keys ===
        # name2param, id2name = _collect_named_params_from_model_list(model)
        # dense_param_groups = list(filter(lambda g: not g['is_expert_parallel'], param_groups))
        # moe_param_groups = list(filter(lambda g: g['is_expert_parallel'], param_groups))

        # print_optimizable_keys("Megatron Optimizer (dense)", dense_param_groups, id2name)
        # if len(moe_param_groups) > 0:
        #     print_optimizable_keys("Megatron Optimizer (MoE)", moe_param_groups, id2name)
        # ===============================================
        ...
        print("mpu.get_data_parallel_group(with_context_parallel=True) =", mpu.get_data_parallel_group(with_context_parallel=True))
        if len(moe_param_groups) > 0:
