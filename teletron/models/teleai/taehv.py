...
class TAEW2_1DiffusersWrapper(nn.Module):
    def __init__(self, taew_path, device, dtype):
        super().__init__()
        # self.dtype = torch.float16
        self.dtype = dtype
        self.device = device
        self.taehv = TAEHV(taew_path).to(device=self.device, dtype=self.dtype)
        ...
        # # flush
        # self.encode(torch.randn(1, 3, 77, 448, 832).to(self.device, self.dtype))
        # self.decode(torch.randn(1, 16, 20, 56, 104).to(self.device, self.dtype))


    def decode(self, latents, **kwargs):
        ori_type = latents.dtype
        n, c, t, h, w = latents.shape
        latents = latents.to(self.dtype)
        # low-memory, set parallel=True for faster + higher memory
        vid_dec = self.taehv.decode_video(latents.transpose(1, 2), parallel=False, show_progress_bar=False).transpose(1, 2)
        vid_dec = vid_dec.mul_(255).add_(0.5).clamp_(0, 255).to(torch.uint8)
        return vid_dec
            
    def single_encode(self, video, **kwargs):
        ori_type = video.dtype
        video = video.permute(0, 2, 1, 3, 4).to(self.dtype) # from ncthw to ntchw
        video = video * 0.5 + 0.5
        out = self.taehv.encode_video(video, show_progress_bar=False)
        out = out.to(ori_type).permute(0, 2, 1, 3, 4) # from ntchw to ncthw
        return out, out
