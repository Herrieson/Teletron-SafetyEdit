...
def forward_step(data_iterator, model, time_step=None):
    ...
    p_mu, p_logvar = batch['latents_masked_images'].chunk(2, dim=1)
    ...
    upsample_latents1, upsample_latents2, upsample_latents3 = model.module.module.compressor_up(dequantized_downsample_latents3, dequantized_downsample_latents2, dequantized_downsample_latents1) # v3
    # upsample_lastents = random.choice([upsample_latents1, upsample_latents2, upsample_latents3]) # three decoder
    upsample_lastents = random.choice([upsample_latents1])
    mu, logvar = upsample_lastents.chunk(2, dim=1)
    ib_loss = kl_gaussian_safe(mu, logvar, p_mu, p_logvar)
    upsample_lastents = reparameterize(mu, logvar) # ib

    rd_loss, distortion, rate = model.module.module.quantizer.compute_loss(
        latents, upsample_lastents, latent_uint8_1, latent_probs1
    )
    ...
    loss_woib = loss
    loss = loss + ib_loss * 1e-5
    loss_word = loss
    loss = loss + rd_loss * 1e-4
    return [loss, loss_woib, loss_word, ib_loss * 1e-5, rd_loss * 1e-4], loss_func
