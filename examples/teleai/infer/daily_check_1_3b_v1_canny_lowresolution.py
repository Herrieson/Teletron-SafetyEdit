...
folder = "/gemini/space/code/data/yifq1/yifq1/workdirs/fl2v_1.3B_recon_multi_resolution_f29_sft_randomcanny_tae_lowresolution_standard/"
...
SAVEDIR = f"/gemini/user/shared/vis_result/20260305/jk_720keyfr"
    ...
    model_config=dict(
        dit=dict(
            ...
            config=dict(
                ...
                has_compressor={"use":True, "up_T":True, "pixel_shuffle": True},
                has_quantizer=True,
            ),
        ),
        encoder=dict(
    ...
    for test_name, config in infer_task:
        ...
        for t in range(NUM_OF_CLIPS):
            try:
                ...
                frames_downsampled = resize_canny_with_pyav(frames_np, target_size=(new_H//2, new_W//2))
                video = torch.from_numpy(frames_downsampled).permute(3, 0, 1, 2).contiguous()  # T,H,W,C → C,T,H,W

                # Load and process canny
                canny = load_canny_cthw_norm(video, new_W//2, new_H//2, canny_cnt)
                video = (video / 255.0 - 0.5) / 0.5
                ori_video = (ori_video / 255.0 - 0.5) / 0.5
                ...
                for k in range(len(result)):
                    vis_image = [
                        to_pil_image(recover(ori_video[:, k].permute(1, 2, 0)).numpy().astype(np.uint8)).crop((0, 0, ori_W, ori_H)),
