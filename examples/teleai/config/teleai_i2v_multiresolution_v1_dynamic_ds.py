...
dst_size = (1408, 768) # (dst // 8) % 8 == 0
buckets_size = [(896, 512), (1408, 768), (2048, 1152)] 
...
config = dict(
    dataset=dict(
        ...
        # + [os.path.join("/gemini/platform/shared/yifq1/yifq/trainable_jsons", x) for x in os.listdir("/gemini/platform/shared/yifq1/yifq/trainable_jsons")],
        # data_path_list = ["/gemini/platform/shared/xujy70/trainable_jsons_new/valid_all_chunk_0.json"],
        filter_cfg=dict(
            dst_size=dst_size,
            dst_num_frames = dst_num_frames,
            dst_fps = [10, 15, 25, 30],
            buckets_size = buckets_size,
            buckets_size_ratio = [0.4, 0.4, 0.2],
            canny_cnt_buckets=list(range(0, 28)),
        ),
        transforms=[
            dict(
                type="SampleImages",
            ...
            dict(
                ...
                cn_keys=["masked_canny", "masked_images"],
            ),
            dict(
                type="PackInputs_TMP",
                crop_keys=[ # 做crop
    ...
    eval=dict(
        # data_path_list = [os.path.join("/gemini/platform/shared/xujingyu/xujy/data/trainable_jsons", x) for x in os.listdir("/gemini/platform/shared/xujingyu/xujy/data/trainable_jsons")]
        # + [os.path.join("/gemini/platform/shared/yifq1/yifq/trainable_jsons", x) for x in os.listdir("/gemini/platform/shared/yifq1/yifq/trainable_jsons")],
        data_path_list = [os.path.join("/gemini/platform/shared/xujy70/trainable_jsons", x) for x in os.listdir("/gemini/platform/shared/xujy70/trainable_jsons")],
    ...
    model_config=dict(
        dit=dict(
            ...
            config=dict(
                ...
                has_compressor={"use":True, "up_T":True, "pixel_shuffle": True, "enable_attn": False},
                has_quantizer=True,
            ),
        ),
        encoder=dict(
            ...
            encoder_schema=['context', 'latents', 'latents_ds', 'img_emb_y_ds', 'img_clip_feature_ds', "latents_canny_images", "latents_masked_images"],
            vae=dict(
